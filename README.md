...
- 預設程式會先嘗試載入「英文檔名」，若不存在會回退載入原始中文檔名（方便上傳到 GitHub 後也能正確讀取）。

預設的中文名稱 → 建議英文檔名對照（你可以把檔案改名為右方英文）：
...
- 若你把上表中的英文檔名放到 `image/` 下，程式會優先載入英文檔名，能避免部分 GitHub 或 Raw 檔案讀取路徑問題。
- 若你不想改檔名，程式仍會在找不到英文檔名時嘗試載入中文檔名作為回退。
- 預設繪製高度為 `HEX * 2`（在 `preview.html` / `index.html` 中 `HEX` 為六角半徑像素），圖片底部會對齊格子並帶有陰影。

若想把圖片放在其他資料夾或使用不同命名，告訴我你要的命名規則，我可以直接把 `CHAR_FILENAME_MAP` 更新為你提供的對照表。
